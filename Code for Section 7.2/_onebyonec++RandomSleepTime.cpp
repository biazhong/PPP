...
void master_receive_tasks(vector<alt>* incoming_alts, int* outgoing_rank){
    MPI_Status status;
    int incoming_rank;
    int incoming_size;
...
void generate_obv (double* _sim, int label){
    ...
    int sleepTime = (int)floor((rand()/(RAND_MAX * 1.0)*2.0+0.0)*1000000);
...
int main(int argc, char** argv){
    ...
    if(world_rank == 0){
        vector<alt> I;
        alt tempAlt;
        for (int i = 0; i < k + world_size - 1; i++){
            tempAlt.label = i+1;
        ...
        while(count <= k + world_size - 1){
            master_receive_tasks(&incoming_alts, &outgoing_rank);
            ...
            incoming_alts.clear();
            outgoing_alts.push_back(I[count-1]);
            master_send_out_tasks(&outgoing_alts, outgoing_rank);
            outgoing_alts.clear();
            count++;
        }
        _endtime = MPI_Wtime();
	recordTime.push_back(-_starttime+_endtime);
        printf("Onebyone: The wall clock time is %f\n", -_starttime+_endtime);
        
        
    }else{
        worker_receive_tasks(&incoming_alts);
        while(incoming_alts[0].label <= k){
    ...
    if(world_rank==0){
	ofstream myfile ("Onebyoneoutput.txt");
