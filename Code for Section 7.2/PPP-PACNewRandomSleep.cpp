...
int main(int argc, char** argv){
    ...
    if(world_rank == 0){
		...
		int sleepTime = (int)floor((rand()/(RAND_MAX * 1.0)*1.0+0.5)*1000000);  //Input Parameter: Random pause time
	...
	printf("%d\n", bestLabel);
        ...
        printf("%d, %f, %f, %f, %f \n",count+1,totalSampleSize,-_starttime+_endtime,total_sim_time,total_comparison_time);
